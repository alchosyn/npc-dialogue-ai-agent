...
> **数据真实性提示**：早期版本的 `evals/run_compare.py` 有个 silent bug（commit `<待跑后填>` 已修）—— qwen-base 和 qwen-lora 共用单一全局模型 cache，第二个策略实际跑的是第一个的模型。所以上表 +0.05 这个数字本身可能是 noise（两路实际是同一模型）。修复后需要重跑才能拿到真实 LoRA 提升数。GRPO 后训练的对比已用修复版跑。
...
| R2 步骤化奖励 | 正则匹配 1.2.3. 或 第一/第二/第三 | +0.5 | 小 bonus |
...
**总分范围 [-1.5, 4]**。LLM judge 看语义（容忍同义词，避免 reward hacking），规则项做形式约束。

### 工程价值（写进简历）

- 这是 **RLAIF (RL from AI Feedback)** 风格而不是纯规则 RL，跟 Constitutional AI / R1 思路一致
- Reward 跟 eval 指标用的是**同一个 LLM judge**，训练直接优化最终评估的目标——是工程闭环
- 单元测试覆盖 9 个 reward 分支（judge 部分用 mock），**纯逻辑可独立验证**
...
| 组件 | 文件 | 状态 |
|---|---|---|
| Reward 函数 | `scripts/grpo_reward.py` | ✅ 单测 9/9 过 |
...
|---|---:|---:|---:|
| deepseek-agent | 4.70 | 3.80 | 5.00 |
| deepseek-base | 3.87 | 1.80 | 5.00 |
| qwen-grpo | _待填_ | _待填_ | _待填_ |
| qwen-lora | _待重跑_ | _待重跑_ | _待重跑_ |
| qwen-base | 3.22 | 1.60 | 4.07 |
...
- [ ] 修复 qwen cache bug 后重跑 4 路对比，拿真实 LoRA 提升数
...
> - **加 GRPO 后训练（DeepSeek-R1 同款 RL 算法）作为 stretch**：在 SFT LoRA 之上叠 GRPO，设计 RLAIF 风格的混合 reward（LLM-as-judge × 0.6 + 4 项规则 bonus/penalty），训练 reward 跟 eval 用同一个 judge 做闭环；脚手架完整（reward 单测 9/9 过 / TRL 版本兼容 / Kaggle notebook 瘦壳 / 5 路 ablation 评估）
