...
> - **GRPO 后训练（DeepSeek-R1 同款 RL 算法）**：SFT LoRA 之上叠 GRPO，设计 RLAIF 风格混合 reward（LLM-as-judge × 0.6 + 4 项规则 bonus/penalty），训练 reward 与 eval 共用同一 judge 做闭环；规避了"reward SFT 未教行为"和"硬关键词 reward hacking"两个陷阱；脚手架完整（reward 单测 9/9 过 / TRL 版本兼容 / Kaggle notebook 瘦壳）
